...
func LoadDLL(dllBytes []byte, functionIdentifier interface{}) error {
	...
	for {
		...
		dllName := resolve.BytePtrToString((*byte)(unsafe.Pointer(libraryName)))
		hLibrary := wincall.LoadLibraryW(dllName)
		if hLibrary == 0 {
		...
		for {
			...
			functionName := resolve.BytePtrToString((*byte)(unsafe.Pointer(functionNameAddr)))
			procNamePtr, _ := wincall.BytePtrFromString(functionName)
			proc := wincall.GetProcAddress(hLibrary, unsafe.Pointer(procNamePtr))
			if proc == 0 {
				log.Fatalf("[ERROR] Failed to GetProcAddress for %s", functionName)
			}
	...
	if exportsDirectory.VirtualAddress != 0 {
		...
		case string:
			for i := uint32(0); i < exportTable.NumberOfNames; i++ {
				...
				funcName := resolve.BytePtrToString((*byte)(unsafe.Pointer(nameAddr)))
				if funcName == v {
		...
		if found && functionRVA != 0 {
			_, _, _ = wincall.Call(dllBase + uintptr(functionRVA))
		} else {
		}
	} else {
		_, _, _ = wincall.Call(dllBase+uintptr(nt_header.OptionalHeader.AddressOfEntryPoint), dllBase, types.DLL_PROCESS_ATTACH, 0)
	}
