...
func resolveForwardedExport(forwarderString string) (uintptr, error) {
	...
	dllHandle := wincall.LoadLibraryW(targetDLL)
	if dllHandle == 0 {
		return 0, fmt.Errorf("[ERROR] failed to load target DLL %s", targetDLL)
	}
	...
	if strings.HasPrefix(targetFunction, "#") {
		...
		funcAddr = wincall.GetProcAddress(dllHandle, unsafe.Pointer(uintptr(ordinal)))
		if funcAddr == 0 {
			return 0, fmt.Errorf("[ERROR] failed to get ordinal %d from %s", ordinal, targetDLL)
		}
	} else {
		funcNameBytes, _ := wincall.BytePtrFromString(targetFunction)
		funcAddr = wincall.GetProcAddress(dllHandle, unsafe.Pointer(funcNameBytes))
		if funcAddr == 0 {
			return 0, fmt.Errorf("[ERROR] failed to get function %s from %s", targetFunction, targetDLL)
		}
	}
...
func fixImportAddressTable(baseAddress uintptr, peFile *pe.File) error {
	...
	for _, importDir := range importDirs {
		...
		dllHandle := wincall.LoadLibraryW(dllName)
		if dllHandle == 0 {
			return fmt.Errorf("[ERROR] failed to load library %s", dllName)
		}
		...
		for {
			...
			procAddr := wincall.GetProcAddress(dllHandle, funcNamePtr)
			if procAddr == 0 {
