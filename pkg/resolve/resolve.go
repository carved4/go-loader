...
func BytePtrToString(p *byte) string {
	...
	// Create a byte slice from the pointer and length
...
func GetModuleBase(moduleHash uint32) uintptr {
	...
	if moduleBase == 0 {
		log.Printf("Module with hash 0x%X not found.\n", moduleHash)
	}
	return moduleBase
}
...
func GetFunctionAddress(moduleBase uintptr, functionHash uint32) uintptr {
	...
	dataSlice := unsafe.Slice((*byte)(unsafe.Pointer(moduleBase)), 20*1024*1024)
